...
export default function LifeScreen() {
  ...
  const salaryStatusLabel = String(workState?.salary_status_label || 'No salary posted').replace(/Ã‚Â·/g, '-');
  const salaryStatusMessage = String(workState?.salary_status_message || 'No salary posted yet.').replace(/Ã‚Â·/g, '-');
  ...
  return (
    <GameplayLoopScaffold
      title="Life"
      subtitle="Daily update, recovery, and home decisions"
      activeNavKey="life"
      footer={guidedLifeActive || !showFooterAction ? null : (
        <GameplayStickyActionArea
    ...
    >
      {loop.dashboard ? (
        <OnboardingHighlight target="life-daily-economy">
          <SlideFadeInOnChange
            watchValue={`life_headline_${workState?.current_game_day || dayLabel}_${loop.dashboard.headline || ''}`}
            delayMs={0}
          >
            <DailyBriefCard
      ...
      {stats ? (
        <SlideFadeInOnChange
          watchValue={`life_stats_${dayLabel}_${Math.round(health)}_${Math.round(stress)}_${Math.round(cash * 100)}_${Math.round(debt * 100)}`}
          delayMs={40}
        >
      ...
      <SlideFadeInOnChange
        watchValue={`life_activity_${workState?.current_game_day || dailyActivity?.day || 1}_${transactions.length}_${Math.round((dailyActivity?.net ?? netFlow) * 100)}`}
        delayMs={80}
      >
